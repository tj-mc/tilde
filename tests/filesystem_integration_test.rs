...
fn test_file_exists_true() {
    ...
    assert_eq!(
        evaluator.get_variable("exists").unwrap().to_string(),
        "true"
    );
...
fn test_file_size() {
    ...
    assert_eq!(
        evaluator.get_variable("size").unwrap().to_string(),
...
fn test_file_size_nonexistent() {
    ...
    assert!(
        error_msg.contains("No such file or directory") || error_msg.contains("file not found")
    );
}
...
fn test_dir_exists() {
    ...
    assert_eq!(
        evaluator.get_variable("nonexistent").unwrap().to_string(),
        "false"
    );
}
...
fn test_filesystem_integration() {
    ...
    assert_eq!(
        evaluator.get_variable("exists_check").unwrap().to_string(),
        "true"
    );
    assert_eq!(
        evaluator.get_variable("size_check").unwrap().to_string(),
        content.len().to_string()
    );

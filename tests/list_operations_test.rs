...
mod predicate_tests {
    ...
    fn test_filter_with_stdlib_predicates() {
        ...
        assert_eq!(result, Value::List(vec![Value::Number(1.0), Value::Number(3.0), Value::Number(5.0)]));
    ...
    fn test_drop_while_with_stdlib_predicates() {
        ...
        assert_eq!(result, Value::List(vec![Value::Number(2.0), Value::Number(7.0)]));
    ...
    fn test_map_with_stdlib_transformers() {
        ...
        assert_eq!(result, Value::List(vec![Value::Number(4.0), Value::Number(9.0), Value::Number(16.0)]));
    }
}
...
mod new_list_functions_tests {
    ...
    fn test_partition_with_stdlib_predicates() {
        ...
        if let Value::Object(map) = result {
            ...
            assert_eq!(*matched, Value::List(vec![Value::Number(2.0), Value::Number(4.0), Value::Number(6.0)]));
            assert_eq!(*unmatched, Value::List(vec![Value::Number(1.0), Value::Number(3.0), Value::Number(5.0)]));
        } else {
    ...
    fn test_group_by_with_numbers() {
        ...
        if let Value::Object(map) = result {
            ...
            assert_eq!(*true_group, Value::List(vec![Value::Number(2.0), Value::Number(4.0), Value::Number(6.0)]));
            assert_eq!(*false_group, Value::List(vec![Value::Number(1.0), Value::Number(3.0), Value::Number(5.0)]));
        } else {
            panic!("group-by should return an object");
        }
    }
    ...
    fn test_sort_by_with_stdlib_functions() {
        ...
        let result = eval_expression(r#"sort-by ["apple", "pie", "banana", "kiwi"] length"#).unwrap();
...
mod collection_functions_tests {
    ...
    fn test_collection_error_handling() {
        ...
        ");
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("append requires exactly two arguments"));
    }
}